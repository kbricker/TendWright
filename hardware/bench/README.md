...
| `runner` | Execute an arbitrary **clip** (a JSON walk through poses, authored in degrees, carrying its own speed/acceleration). `example` prints a starter file, `show` prints the plan and gates it without touching the arm, `run` executes it. Gates the WHOLE clip up front — approach to pose 0 included — then re-gates every edge from the arm's measured pose, because settle tolerance permits up to ~26 mm of tool drift against the gate's 5 mm margin. A stop mid-clip halts and HOLDS: it never auto-returns, because the path back was never gated. This is the executor a computed pick-and-place sequence runs on. |
| `exercise` | Scripted limber-up from `calibration.json`: wake (no lurch) → rest pose → sweep each joint through a sub-range of its calibrated span, one at a time, distal first (4→5→6→3→2→1 — rest is a compact fold, so the wrist unfolds before the heavy joints move). The elbow (3) and wrist (4) hold 90° open while the shoulder (2) sweeps — refolded, they would be pressed into the table — then refold. The routine IS a clip (`exercise_clip`) and `runner` executes it, so what the twin simulated is what the servos are told; the holds travel with it and are verified from the encoders, not assumed from the command. Ends at rest, torque off. Refuses without a calibration or away from rest; ANY key = e-stop. `--selftest` checks the one-definition property with no arm attached. |
| `posemachine` | The pose library as a state machine: states are named poses, edges are moves, and a move may only fire when the twin has validated that edge AND the encoders confirm the arm is at the source pose. An aborted move lands in `BETWEEN`, which authorises nothing — `jog` out of it, then `resync`. `show` prints the graph and which guards are armed; `validate` asks the twin about EVERY edge with no arm attached, so a move the geometry forbids is a line of output rather than a refusal you discover standing at the bench. Neither opens the bus. |
| `campreview` | Live camera window with tag36h11 AprilTag overlay + FPS; `--grab N` for headless snapshots. Paced to `--fps` (default 30) — these cameras ignore the rate you request and will run 120 fps otherwise. |
...
| `camserve` | The camera bus: every registered camera from one server. `/` picker, `/all` 3×3 tiles, `/cam/<name>/` full-res view, `/cam/<name>/snapshot` (curl-able), `/status` JSON. Cameras open only while watched; cameras with `still_interval_s` write full-res stills to `stills/<name>/` with no viewer attached. Tag detection is **opt-in per request** — add `?tags=1` or use the toggle on the solo page; plain viewing never loads the detector (`tags:` in cameras.json and `--no-tags` can only forbid it, never enable it). No auth — home LAN only, never port-forward. Never touches the servo bus. |
