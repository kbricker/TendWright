...
from .campreview import TAG_FAMILY, annotate, open_camera  # noqa: E402
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):  # noqa: N802 (http.server API name)
            try:
                ...
                pass  # client left mid-write — normal, never fatal
        ...
        def _stream(self) -> None:
            ...
            while True:
                ...
                if jpeg is None:
                    if box.closed:
                        return
                    continue  # camera hiccup — keep the client waiting
...
def capture_loop(cap: cv2.VideoCapture, box: FrameBox,
    ...
    Runs on the MAIN thread so Ctrl+C lands here."""
    times: list[float] = []
    while True:
        ok, frame = cap.read()
        if not ok:
...
def run() -> int:
    ...
    print(f"serving http://{host}:{args.listen}/  "
          f"(/stream, /snapshot; tags {'off' if args.no_tags else 'on'})")
    print("LAN only, no auth — never port-forward this. Ctrl+C to stop.")
    try:
        capture_loop(cap, box, detector)
        return 0  # unreachable; capture_loop exits via exception only
    finally:
